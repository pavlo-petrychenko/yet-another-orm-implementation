import type {ColumnDescription} from "@/query-builder";
...
export class PostgresDialectUtils {
  ...
  private escapeQualifiedColumn(col: ColumnDescription): string {
    ...
    return col.alias
      ? `${this.escapeIdentifier(table)}.${this.escapeIdentifier(name)} AS ${this.escapeIdentifier(col.alias)}`
      : `${this.escapeIdentifier(table)}.${this.escapeIdentifier(name)}`;
  }
  ...
  private escapeSimpleColumn(col: ColumnDescription): string {
    if (col.table) {
      ...
      return col.alias
        ? `${this.escapeIdentifier(col.name)} AS ${this.escapeIdentifier(col.alias)}`
