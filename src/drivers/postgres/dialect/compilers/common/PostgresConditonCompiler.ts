...
import type {BaseCondition, ConditionClause, ConditionGroup, RawCondition} from "@/query-builder";
...
export class PostgresConditionCompiler {
  ...
  compile(condition: ConditionClause): CompiledQuery {
    ...
    this.logger.debug({condition, timestamp: new Date().toISOString()}, "Starting compilation");

    try {
      let result: CompiledQuery;
      if (condition.conditionType === "condition") {
        result = this.compileBaseCondition(condition as BaseCondition);
      } else if (condition.conditionType === "raw_condition") {
      ...
      if (error instanceof Error) {
        ...
        this.logger.error({condition, error: error.message, stack: error.stack}, "Compilation failed: ");
        throw error;
      }
  ...
  private compileConditionGroup(condition: ConditionGroup): CompiledQuery {
    const startTime = Date.now();
    // Log compilation details
    this.logger.debug({condition, timestamp: new Date().toISOString()}, "Compiling condition group");
    ...
    try {
      ...
      // Log timing information
      this.logger.debug("Compilation completed in %dms", duration);
      this.logger.debug({sql: `(${parts.join("")})`, params: allParams, duration}, "Condition group compiled: ");
      ...
      if (error instanceof Error) {
        // Log error information
        this.logger.error({condition, error: error.message, stack: error.stack}, "Error compiling condition group: ");
        throw error;
      }
      throw new Error("Unknown error occurred during compilation", {cause: error});
    }
  ...
  private compileBaseCondition(cond: BaseCondition): CompiledQuery {
    const startTime = Date.now();
    // Log compilation details
    this.logger.debug({cond, timestamp: new Date().toISOString()}, "Compiling base condition");
    ...
    try {
      ...
      if (Array.isArray(right)) {
        const placeholders = right.map(() => this.paramManager.getNextParameter()).join(", ");
      ...
      if (error instanceof Error) {
        // Log error information
        this.logger.error({cond, error: error.message, stack: error.stack}, "Error compiling base condition: ");
        throw error;
      }
      throw new Error("Unknown error occurred during compilation", {cause: error});
    }
