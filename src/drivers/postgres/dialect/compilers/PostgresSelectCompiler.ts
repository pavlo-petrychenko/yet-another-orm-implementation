...
import type {Query, ColumnDescription, JoinClause, OrderByClause, RawExpression, SelectQuery} from "@/query-builder";
...
export class PostgresSelectCompiler extends PostgresQueryCompiler {
  ...
  compile(query: Query): CompiledQuery {
    ...
    this.logger.debug({query, timestamp: new Date().toISOString()}, "Starting select query compilation");
    try {
      ...
      if (query.type !== "SELECT") {
        ...
        this.logger.error({query, error: error.message, stack: error.stack}, "Select query compilation failed: ");
        throw error;
      }
      if (query.distinct) {
        parts.push("DISTINCT");
      ...
      this.logger.debug({sql: parts.join(" "), params, duration}, "Select query compiled successfully: ");
      return {sql: parts.join(" "), params};
    } catch (error) {
      if (error instanceof Error) {
        // Log error information
        this.logger.error({query, error: error.message, stack: error.stack}, "Failed to compile select query: ");
        throw error;
      }
  ...
  private addColumns(
    ...
    const escapedCols = columns.length > 0 ? columns.map((col) => this.dialectUtils.escapeIdentifier(col)) : ["*"];
  ...
  private addOrderByClause(parts: string[], order: OrderByClause | undefined): void {
    ...
    parts.push(
      order.orders
        .map((clause) => {
          return `${this.dialectUtils.escapeIdentifier(clause.column)} ${clause.direction}`;
        })
        .join(", "),
    );
  }
   ...
   */
  private addUnionClauses(parts: string[], params: any[], unions?: {query: SelectQuery; all: boolean}[]): void {
