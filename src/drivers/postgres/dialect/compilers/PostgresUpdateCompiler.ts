...
export class PostgresUpdateCompiler extends PostgresQueryCompiler {
  ...
  compile(query: Query): CompiledQuery {
    ...
    this.logger.debug({query, timestamp: new Date().toISOString()}, "Starting update query compilation");
    try {
      ...
      if (query.type !== "UPDATE") {
        ...
        this.logger.error({query, error: error.message, stack: error.stack}, "Update query compilation failed: ");
        throw error;
      }
      ...
      this.logger.debug({sql: parts.join(" "), params, duration}, "Update query compiled successfully: ");
      return {sql: parts.join(" "), params};
    } catch (error) {
      if (error instanceof Error) {
        // Log error information
        this.logger.error({query, error: error.message, stack: error.stack}, "Failed to compile update query: ");
        throw error;
      }
