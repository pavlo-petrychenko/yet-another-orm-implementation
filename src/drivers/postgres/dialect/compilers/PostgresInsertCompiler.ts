...
export class PostgresInsertCompiler extends PostgresQueryCompiler {
  ...
  compile(query: Query): CompiledQuery {
    ...
    this.logger.debug({query, timestamp: new Date().toISOString()}, "Starting insert query compilation");
    ...
    try {
      ...
      if (query.type !== "INSERT") {
        ...
        this.logger.error({query, error: error.message, stack: error.stack}, "Insert query compilation failed: ");
        throw error;
      }
      ...
      this.logger.debug({sql: parts.join(" "), params, duration}, "Insert query compiled successfully: ");
      return {sql: parts.join(" "), params};
    } catch (error) {
      if (error instanceof Error) {
        // Log error information
        this.logger.error({query, error: error.message, stack: error.stack}, "Failed to compile insert query: ");
        throw error;
      }
