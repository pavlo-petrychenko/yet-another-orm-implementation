...
export class PostgresDeleteCompiler extends PostgresQueryCompiler {
  ...
  compile(query: Query): CompiledQuery {
    ...
    this.logger.debug({query, timestamp: new Date().toISOString()}, "Starting delete query compilation");
    try {
      ...
      if (query.type !== "DELETE") {
        ...
        this.logger.error({query, error: error.message, stack: error.stack}, "Delete query compilation failed: ");
        throw error;
      }
      ...
      this.logger.debug({sql: parts.join(" "), params, duration}, "Delete query compiled successfully: ");
      return {sql: parts.join(" "), params};
    } catch (error) {
      if (error instanceof Error) {
        // Log error information
        this.logger.error({query, error: error.message, stack: error.stack}, "Failed to compile delete query: ");
        throw error;
      }
